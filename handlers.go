...
func (h handler) loadAccountsByPw(ctx context.Context, accts AccountCollection, pw string) Account {
	config := h.storage.GetOauth2Config(fedboxProvider)
...
func (h *handler) ShowOAuthClientIdentityMetadata(w http.ResponseWriter, r *http.Request) {
	...
	m := box.ClientRegistrationRequest{
		ClientName:              h.conf.HostName,
		TokenEndpointAuthMethod: "none",
		GrantTypes:              []string{"authorization_code", "refresh_token", "client_credentials"},
		ResponseTypes:           []string{"token", "code"},
		ClientURI:               h.conf.BuildOAuth2ClientURL(),
		Contacts:                []string{h.conf.AdminContact, "https://lists.sr.ht/~mariusor/go-activitypub-discuss"},
		TosURI:                  h.conf.BaseURL + "/about",
		RedirectUris:            []string{h.conf.buildOAuth2RedirectURL()},
		SoftwareID:              uid.String(),
	}

	app := h.storage.app
	if app.HasIcon() {
		m.LogoURI = app.Metadata.Icon.URI
	}
